...
public class WorldSender
{
    ...
    void SendUrgent()
    {
        for(Player receiver : Player.connected.values())
        {
            if(receiver.canReceive())
            {
                sendUrgentNPCHeadingsToReceiver(receiver);
                sendUrgentPlayerHeadingsToReceiver(receiver, false);
            }
        }
    }

    private void sendUrgentNPCHeadingsToReceiver(Player receiver)
    {
        //TODO: interest management for npc info sending
        ActionscriptObject res = new ActionscriptObject();
        res.put("_cmd","h");
        ...
        for(NPC npc : NPC.all)
        {
            if(!npc.headingHasChanged(receiver) &&!npc.isAccelerating())
            {
                continue;
            }
    ...
    synchronized public void sendUrgentPlayerHeadingsToReceiver(Player receiver, boolean forceSend)
    {
        //TODO: interest management for player info sending
        ActionscriptObject res = new ActionscriptObject();
        res.put("_cmd","h");
        ...
        for(Player p : Player.connected.values())
        {
            if(p.getUser().getUserId() != receiver.getUser().getUserId())
            {
                if(!forceSend && !p.headingHasChanged(receiver) &&!p.isAccelerating())
                    continue;
                double x = (double) p.getX();
                double y = (double) p.getY();
                double z = (double) p.getZ();
